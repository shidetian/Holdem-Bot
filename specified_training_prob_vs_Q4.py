...
from realizedStatus import AnotherStatus
...
for LAMB in [0.6, 0.65,0.7]:
    name = ('specified_training_vs_Q'+str(num)+'_.005_' + 
            str(LAMB)+'_' +str(check_prob)
            +'_'+ str(call_prob) +'_'+ str(raise_prob) 
            +'_'+str(checkfold_prob) +'realized'+'.p')
    print n_cards
    ...
    ai = specified_prob.Specified_prob(prob_list={
            'Check':list_of_bots[num+2][0], 'Call':list_of_bots[num+2][1], 
